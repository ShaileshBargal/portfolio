  ...
  return (
    <header
      className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${
        scrolled
          ? 'bg-white/85 dark:bg-slate-950/85 backdrop-blur-xl border-b border-slate-200/80 dark:border-slate-800/80 shadow-md shadow-slate-950/5'
          : 'bg-transparent'
      }`}
    >
            ...
            {navLinks.map(link => {
              ...
              return (
                <button
                  key={link.href}
                  onClick={() => handleNav(link.href)}
                  className={`w-full text-left px-4 py-2.5 rounded-xl text-base font-semibold transition-all cursor-pointer ${
                    isActive
